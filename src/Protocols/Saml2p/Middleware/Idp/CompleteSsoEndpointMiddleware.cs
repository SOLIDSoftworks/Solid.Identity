    ...
    internal class CompleteSsoEndpointMiddleware : Saml2pEndpointMiddleware
    {
        ...
        private async Task CompleteSsoAsync(HttpContext context)
        {
            ...
            else if (authentication.Succeeded && authentication.Principal?.Identity is ClaimsIdentity identity)
            {
                ...
                using (Tracing.Saml2p.StartActivity($"{nameof(Saml2pIdentityProviderEvents)}.{nameof(Saml2pIdentityProviderEvents.OnCreatingSecurityToken)}"))
                    await Events.InvokeAsync(Options, partner, e => e.OnCreatingSecurityToken(context.RequestServices, createSecurityTokenContext));

                using (Tracing.Saml2p.StartActivity($"{nameof(Saml2SecurityTokenHandler)}.{nameof(Saml2SecurityTokenHandler.CreateToken)}"))
                ...
                using (Tracing.Saml2p.StartActivity($"{nameof(Saml2pIdentityProviderEvents)}.{nameof(Saml2pIdentityProviderEvents.OnCreatedSecurityToken)}"))
            ...
            using (Tracing.Saml2p.StartActivity($"{nameof(Saml2pIdentityProviderEvents)}.{nameof(Saml2pIdentityProviderEvents.OnCompleteSso)}"))
